...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!resendApiKey) {
      ...
      return NextResponse.json({ 
        success: true, 
        message: 'Contact form submitted successfully (logged to console - email service not configured)'
      });
    }
    ...
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: process.env.RESEND_FROM_EMAIL || 'Consently <onboarding@resend.dev>',
        to: process.env.CONTACT_EMAIL || 'hello@consently.in',
        reply_to: email,
