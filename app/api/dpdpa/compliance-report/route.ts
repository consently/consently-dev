...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch complete user profile for report personalization
    const { data: userProfile, error: profileError } = await supabase
      .from('users')
      .select('full_name, email, company_name, phone, website')
      .eq('id', user.id)
      .single();
    ...
    if (format === 'csv') {
      // Generate CSV
      const companyName = userProfile?.company_name || 'Your Organization';
      const contactInfo = [userProfile?.email, userProfile?.phone, userProfile?.website].filter(Boolean).join(' | ');
      ...
      const csvRows = [
        ...
        [],
        ['Report Information'],
        ['Report ID', reportData.reportMetadata.reportId],
        ['Generated At', new Date(reportData.reportMetadata.generatedAt).toLocaleString()],
        ['Generated By', reportData.reportMetadata.generatedBy.userName],
        ['Email', reportData.reportMetadata.generatedBy.userEmail],
