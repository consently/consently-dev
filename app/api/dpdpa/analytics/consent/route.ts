...
import type { ConsentEvent } from '@/types/dpdpa-widget.types';

/**
 * Public API endpoint to track consent events
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rateLimitResult = checkRateLimit({
      max: 100, // 100 consent events per minute per IP
    ...
    // Find corresponding consent record if available (by visitor_id and widget_id, most recent)
    ...
    const { error: insertError } = await supabase
      ...
      .insert({
        ...
        consent_record_id: consentRecord?.id || null,
      });

    if (insertError) {
      console.error('[Analytics API] Error inserting consent event:', insertError);
      return NextResponse.json(
        { 
