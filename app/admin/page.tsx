...
export default function AdminPanel() {
  ...
  const [emailVerificationData, setEmailVerificationData] = useState<EmailVerificationMetrics | null>(null);
  const [emailVerificationLoading, setEmailVerificationLoading] = useState(false);
  const [emailVerificationError, setEmailVerificationError] = useState<string | null>(null);
  const [emailDateRange, setEmailDateRange] = useState<string>('30');
  const [selectedEmailWidget, setSelectedEmailWidget] = useState<string>('all');
  const [selectedEmailUser, setSelectedEmailUser] = useState<string>('all');
  const [emailWidgets, setEmailWidgets] = useState<Array<{ value: string; label: string }>>([
    { value: 'all', label: 'All Widgets' },
  ]);
  const [emailUsers, setEmailUsers] = useState<Array<{ value: string; label: string }>>([
  ...
  const fetchEmailVerificationData = async () => {
    ...
    try {
      ...
      if (selectedEmailUser !== 'all') {
        params.append('userId', selectedEmailUser);
      }

      if (selectedEmailWidget !== 'all') {
        params.append('widgetId', selectedEmailWidget);
      }
      ...
      setEmailVerificationData(result.data);

      // Update widgets dropdown based on current filters
      if (result.data.byWidget.length > 0) {
        const widgetOptions = [
          { value: 'all', label: 'All Widgets' },
          ...result.data.byWidget.map((w: any) => ({
            value: w.widgetId,
            label: w.widgetName || w.widgetId,
          })),
        ];
        setEmailWidgets(widgetOptions);
      } else {
        // Reset to default if no widgets
        setEmailWidgets([{ value: 'all', label: 'All Widgets' }]);
      }
    } catch (err) {
      console.error('Error fetching email verification analytics:', err);
  ...
  useEffect(() => {
    if (isAuthenticated && activeTab === 'email-verification') {
      // Fetch users list if not already loaded
      if (emailUsers.length === 1 && data?.users) {
        const userOptions = [
          { value: 'all', label: 'All Users' },
          ...data.users.map(user => ({
            value: user.id,
        ...
        setEmailUsers(userOptions);
      }
      fetchEmailVerificationData();
    }
  }, [isAuthenticated, activeTab, emailDateRange, selectedEmailWidget, selectedEmailUser]);
  ...
  const navigation = [
    ...
    { name: 'Email Verification', icon: Mail, id: 'email-verification' },
  ];
          ...
          {activeTab === 'email-verification' && (
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                ...
                <Button onClick={fetchEmailVerificationData} disabled={emailVerificationLoading} variant="outline">
                  <RefreshCw className={`mr-2 h-4 w-4 ${emailVerificationLoading ? 'animate-spin' : ''}`} />
                  Refresh
                </Button>
              </div>
                    ...
                    {emailUsers.length > 1 && (
                      <div className="flex items-center gap-4 flex-1 min-w-[200px]">
                        ...
                        <Select
                          ...
                          onChange={(e) => {
                            setSelectedEmailUser(e.target.value);
                            // Reset widget filter when user changes
                            setSelectedEmailWidget('all');
                          }}
                          options={emailUsers}
                        />
                      </div>
                    )}
                    {emailWidgets.length > 1 && (
                      <div className="flex items-center gap-4 flex-1 min-w-[200px]">
                        <label className="text-sm font-medium text-gray-700">Widget:</label>
                        <Select
                          value={selectedEmailWidget}
                          onChange={(e) => setSelectedEmailWidget(e.target.value)}
                          options={emailWidgets}
                        />
                      </div>
                    )}
                  </div>
                  ...
                  <CardContent className="pt-6">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-red-700 mt-1">{emailVerificationError}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
              ...
              {/* Loading State */}
              {emailVerificationLoading && !emailVerificationData && (
              ...
              {/* Overview Metrics */}
              {!emailVerificationLoading && !emailVerificationError && emailVerificationData && (
                <>
                  <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
                    <Card>
                      ...
                      <CardContent>
                        <div className="text-2xl font-bold">{emailVerificationData.overview.totalOtpSent.toLocaleString()}</div>
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <div className="text-2xl font-bold text-green-600">
                          {emailVerificationData.overview.totalVerified.toLocaleString()}
                        </div>
                        <p className="text-xs text-gray-600 mt-1">
                          {emailVerificationData.overview.verificationRate.toFixed(1)}% success rate
                        </p>
                      </CardContent>
                    </Card>
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <div className="text-2xl font-bold text-yellow-600">
                          {emailVerificationData.overview.totalSkipped.toLocaleString()}
                        </div>
                        <p className="text-xs text-gray-600 mt-1">
                          {emailVerificationData.overview.skipRate.toFixed(1)}% skip rate
                        </p>
                      </CardContent>
                    </Card>
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <div className="text-2xl font-bold">
                          {emailVerificationData.overview.averageTimeToVerifySeconds < 60
                            ? `${emailVerificationData.overview.averageTimeToVerifySeconds}s`
                            : `${Math.floor(emailVerificationData.overview.averageTimeToVerifySeconds / 60)}m ${emailVerificationData.overview.averageTimeToVerifySeconds % 60}s`}
                        </div>
                        <p className="text-xs text-gray-600 mt-1">
                  ...
                  <div className="grid gap-6 md:grid-cols-3">
                    <Card>
                      ...
                      <CardContent>
                        <div className="text-2xl font-bold text-red-600">
                          {emailVerificationData.overview.totalFailed.toLocaleString()}
                        </div>
                        <p className="text-xs text-gray-600 mt-1">
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <div className="text-2xl font-bold text-orange-600">
                          {emailVerificationData.overview.totalRateLimited.toLocaleString()}
                        </div>
                        <p className="text-xs text-gray-600 mt-1">
                          Blocked requests
                        </p>
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <div className="text-2xl font-bold text-green-600">
                          {emailVerificationData.overview.totalOtpSent > 0
                            ? (((emailVerificationData.overview.totalVerified) / emailVerificationData.overview.totalOtpSent) * 100).toFixed(1)
                            : 0}%
                        </div>
                        <p className="text-xs text-gray-600 mt-1">
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <ResponsiveContainer width="100%" height={350}>
                        <LineChart data={emailVerificationData.timeSeries}>
                  ...
                  {/* By Widget Breakdown */}
                  {emailVerificationData.byWidget.length > 0 && (
                    <Card>
                      <CardHeader>
                      ...
                      <CardContent>
                        <div className="overflow-x-auto">
                          <table className="w-full border-collapse">
                            ...
                            <tbody>
                              {emailVerificationData.byWidget.map((widget) => (
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="space-y-2">
                        {emailVerificationData.recentEvents.length === 0 ? (
                          <p className="text-center text-gray-500 py-8">No recent events</p>
                        ) : (
                          emailVerificationData.recentEvents.map((event) => {
