-- Emergency fix: Check and seed purposes if missing
-- Run this if you're getting "Invalid purpose ID" errors

-- First, check if purposes exist
DO $$ 
DECLARE
  purpose_count INTEGER;
BEGIN
  SELECT COUNT(*) INTO purpose_count FROM purposes WHERE is_predefined = TRUE;
  
  IF purpose_count = 0 THEN
    RAISE NOTICE 'No predefined purposes found. Seeding purposes...';
    ...
    RAISE NOTICE 'Purposes seeded successfully!';
  ELSE
    RAISE NOTICE 'Found % predefined purposes. No seeding needed.', purpose_count;
  END IF;
END $$;
